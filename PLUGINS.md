...
|---|---|---|---|---|
| `ai-innovisory-base-skills` | General-purpose base Claude Cowork skills | 0.7.1 | research-review, transcript-clean, file-cleanup | 2026-07-15 |
...
| 0.7.1 | 2026-07-16 | `file-cleanup`: two safety fixes to the mutation scripts, plus a test suite. `move_and_index.py` now fully resolves every destination and refuses (skips with a stderr warning) any that would land outside the archive folder, whether from an absolute `dest_relative` or `..` traversal, so the deterministic script is a genuine backstop against a bad manifest rather than a passthrough. `index.md` writing now escapes backslashes and pipes in the Original location, Archived to, and Topic columns, and skips any file whose name contains a newline or other control character; `restore_from_index.py` splits rows on unescaped pipes only and reverses the escaping, so a filename containing a pipe now archives and undoes cleanly (previously its row corrupted the table and the file could never be restored). Indexes written by earlier versions, both 5-column and raw 6-column, still parse and restore unchanged. Added `test_file_cleanup.py` covering containment, pipe round-trips, collision suffixes, undo row removal, and old-format indexes. `transcript-clean`: corrected the frontmatter description, which claimed a single readable markdown file and omitted batch mode; the skill produces two files (cleaned transcript plus separate summary) and handles a folder of transcripts, as the skill body already said. |
